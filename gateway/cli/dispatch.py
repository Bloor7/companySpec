...
# §36 — approvals đã chuyển sang core/policy/. Thêm đúng thư mục chứa nó
# thay vì import dạng gói, để phần còn lại của file không phải đổi.
sys.path.insert(0, os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__)))), "core", "policy"))
...
def load_spec(company_id: str) -> dict:
    ...
    if not os.path.isfile(path):
        ...
        raise FileNotFoundError(
            ...
            f"Chạy `python3 gateway/cli/dispatch.py list` để xem từng company làm được gì."
        )
    with open(path, encoding="utf-8") as fh:
