...
"""travis — cửa vào Travis Core cho admin.
    ...
    python3 gateway/cli/travis.py health              # hệ có còn sống không, im lặng bao lâu
    ...
    python3 gateway/cli/travis.py employees           # ai làm được gì, và ai bị cấm gì
    python3 gateway/cli/travis.py brains <mức>        # mức dữ liệu này gửi được cho ai
    python3 gateway/cli/travis.py mission list|new|close
    python3 gateway/cli/travis.py memory recall|forget-expired
    python3 gateway/cli/travis.py acquire <đường dẫn> # soi một repo lạ, KHÔNG chạy nó
    python3 gateway/cli/travis.py verify              # tự kiểm repo này
...
def cmdAcquire(args) -> int:
    ...
    from core.lab.acquisition import formatReport, inspectRepository
