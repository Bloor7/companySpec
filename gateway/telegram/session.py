...
được và test được mà không cần bot nào cả.
    ...
    echo '<update json>' | python3 gateway/telegram/session.py handle
...
# §36 — approvals sang core/policy/, bộ não dự phòng sang brains/.
_repoRoot = os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_repoRoot, "core", "policy"))
sys.path.insert(0, os.path.join(_repoRoot, "brains"))
...
def ceo_store():
    ...
    conn.executescript(
        ...
        -- `python3 gateway/telegram/session.py soat-so-tay`.
...
def run_nao(*argv) -> str:
    """Chạy brains/fallback.py trong tiến trình riêng rồi lấy chữ nó in ra.
    ...
    """
    script = os.path.join(ROOT, "brains", "fallback.py")
    try:
        proc = subprocess.run([sys.executable, script, *argv],
        ...
        return ("Lệnh nao chạy quá 3 phút nên em cắt. Nhà cung cấp nào đó đang "
                "treo — chạy python3 brains/fallback.py kiem trên máy để xem cái nào.")
    return (proc.stdout or proc.stderr).strip()[:3500]
...
def stop_everything() -> str:
    ...
    # `gateway/cli/dispatch.py` ở tiến trình con, và mẫu dưới đây bắt đúng nó. Cái còn
    # chạy tiếp chỉ là một lời gọi HTTP tới nhà cung cấp model.
    killed = []
    for pattern in ("claude -p", "gateway/cli/dispatch.py"):
...
def danh_muc_block() -> str:
    ...
    return ("\n\n## Danh mục company\n\n"
            ...
            "Còn thiếu gì nữa thì chạy `python3 gateway/cli/dispatch.py list`.\n\n"
...
def _muc_rieng_tu(cfg=None) -> str:
    try:
        ...
        print(f"[gateway] không đọc được fallback.riengTu: {exc}", file=sys.stderr)
        muc = None
...
def _chay_phu(message: str, session_id: str, system_prompt: str,
    ...
    data = fallback.chay(message, system_prompt=system_prompt, lich_su=lich_su,
...
def run_ceo(message: str, session_id: str, resume: bool,
    ...
    BA CHẾ ĐỘ, xem `python3 brains/fallback.py trang-thai`. Chế độ `tu` là thứ đáng nói:
...
def khoi_media(msg: dict) -> tuple:
    ...
    Ảnh được một tiến trình riêng đọc hộ (gateway/telegram/media.py) vì CEO không có quyền mở
    ...
    if cfg.get("adminChatId") in (None, ""):
        return [{"kind": "log", "level": "error",
                 "text": "adminChatId chưa cấu hình — đặt COMPANYSPEC_ADMIN_CHAT_ID "
                         "hoặc registry/session.local.yaml (F6). Không xử lý gì."}]
...
def chay_viec_da_duyet(row, approval_id: str) -> dict:
    ...
    Đi qua ĐÚNG cổng thật (`gateway/cli/dispatch.py`) như mọi lời gọi khác — T2 không
    có ngoại lệ nào, kể cả cho session. Chỉ khác một chỗ: nội dung lấy từ sổ
    ...
    if args.cmd == "check":
        ...
        if cfg.get("adminChatId") in (None, ""):
            problems.append("adminChatId chưa đặt (COMPANYSPEC_ADMIN_CHAT_ID "
                            "hoặc registry/session.local.yaml)")
