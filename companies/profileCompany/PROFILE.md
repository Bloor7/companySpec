# Hồ sơ admin

<!-- Sinh và đọc bởi profileCompany. Sửa tay thoải mái — company đọc lại đúng
     file này, không giữ bản sao ở đâu khác.
     Mỗi dòng dạng:  - (id) nội dung
                     - (id) [đến YYYY-MM-DD] nội dung   ← chỉ đúng tới ngày đó
     Nội dung ở đây được nạp vào system prompt của CEO ở mỗi lượt — trừ những
     dòng đã quá ngày ghi trong ngoặc vuông, chúng nằm lại đây nhưng CEO không
     còn đọc. Muốn dùng lại thì sửa ngày. -->

## thói quen
- (p1) Dậy 5h30 mỗi ngày để mở cửa.
- (p3) Khi ghi thu/chi mà không nói rõ ví thì mặc định là tiền mặt. Nếu kèm "ck" thì là chuyển khoản, cộng/trừ vào ví tài khoản.

## bối cảnh
- (p2) Có web panharmon.com, hướng kinh doanh: SEO ngách giải mộng + quảng cáo, kết hợp làm content TikTok/Reels/YouTube Shorts để kiếm tiền view và affiliate
- (p4) Quản lý khách sạn nhỏ ở Đà Lạt. Lương = 10% doanh thu khách sạn. Có thêm thu nhập từ bán mì, nước ngọt và cho thuê xe máy cá nhân giá 120.000đ/ngày.
- (p5) Remotion (trình edit video bằng React/code) đang thử kết hợp với Claude để làm video
