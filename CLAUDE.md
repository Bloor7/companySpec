...
Cấu trúc theo **§35 của kế hoạch Travis**. `ops/` KHÔNG còn là tầng điều phối —
xem [ops/README.md](ops/README.md) để biết mọi thứ đã đi đâu.

| Tầng | Chứa gì | Được phụ thuộc vào |
|---|---|---|
| `lib/` | Kỹ thuật thuần: mở SQLite, gọi Notion, gọi HTTP | **không gì cả** |
| `core/` | **LUẬT**: `task` `policy` `permissions` `registry` `execution` `verification` `memory` `secrets` `events` `audit` `missions` `lab` | **chỉ `core/` + `lib/`** (C4.2) |
| `brains/` | Bộ não, thay được: `router` `council` `claude` `gemini` `gpt` `local` `fallback` | `core/`, `lib/` |
| `gateway/` | **CỬA VÀO**: `telegram/` (poller, session, cầu) và `cli/` (dispatch, travis, scheduler) | `core/`, `brains/`, `lib/` |
| `companies/<x>/src/` | Nghiệp vụ một lĩnh vực. Mỗi company là hộp kín, có sổ riêng | **chỉ `lib/`** + `brainRunner` |
| `backOffice/src/` | Theo dõi, báo cáo. Hạ tầng, KHÔNG phải company | `lib/`, `core/policy` |
| `employees/<x>/` | Danh tính: vai, tính cách, quyền CÓ PHẠM VI, `cannot` cứng | (dữ liệu) |
| `projects/<x>.yaml` | Nơi làm việc: repo, nhánh bảo vệ, môi trường | (dữ liệu) |
| `missions/` | Mục tiêu dài hạn | (dữ liệu) |
| `lab/` ↔ `main/` | Thử cái mới ↔ vùng tin được. `lab/` **được phép hỏng** (P6) | (dữ liệu) |
| `ops/` | Công cụ cho NGƯỜI PHÁT TRIỂN: codemap, namingAudit, snapshot | không nằm trên đường chạy |
| `ceo/playbooks/` | Sổ tay của CEO — nạp theo việc. Router `session.chon_so_tay()` chọn bằng từ khoá, code cứng nằm NGOÀI model | (chữ) |

**`core/` khác `gateway/` ở đúng một chỗ, và đó là cả điểm của việc tách:**

- `core/` **QUYẾT ĐỊNH** — hàm thuần, không I/O, cùng đầu vào ra cùng câu trả
  lời. Kiểm được bằng một dòng `assert`.
- `gateway/` **TRA sự thật rồi THI HÀNH** — mở sqlite, tra whitelist, tiêu
  phiếu duyệt, chạy tiến trình con, nói chuyện với Telegram.
...
> ⚠ Đừng đặt module tên trùng tên gói. Có `gateway/` và `gateway.py` cùng lúc
> thì Python thấy thư mục trước và `import gateway` trả về một gói **rỗng** —
> không lỗi lúc nạp, chỉ vỡ ở chỗ dùng đầu tiên. Vì vậy module ấy tên
> `session.py`.
...
| **Mảng object mà danh mục chỉ ghi "array"** | CEO gửi `["chuỗi"]` thay vì `[{...}]` → bị chặn → gọi lại. Một việc tốn 4 lời gọi thay vì 2. Kết quả cuối vẫn đúng nên không ai để ý | Danh mục trong prompt phải kể tên trường bên trong (`session.danh_muc_block`). **Ca thử `viec-vat-vao-todo` canh chỗ này** |
...
| **Lỗi hạ tầng lặp lại nhắn mỗi lần** | Cron 15 phút/lần × sự cố 6 tiếng = 21 tin giống hệt lúc nửa đêm, dạy admin bỏ qua thông báo | Lỗi lần đầu thì nhắn, lặp thì im, khỏi thì báo kèm số lần (`core/events/scheduler.py`) |
...
| **Tên model cũ đi trong vài ngày** | Viết `registry/models.yaml` ngày 27/08, tới 31/08 thì groq không còn llama-3.x nào, openrouter đẩy deepseek-v3 khỏi gói free, `mistral-large` rời tier miễn phí. Không nhà nào báo ai | Đừng chép tên model từ tài liệu hay trí nhớ — hỏi thẳng `/models` của nhà đó. `python3 brains/fallback.py kiem` chạy đúng phép hỏi ấy, rẻ, và là cách duy nhất biết chuỗi dự phòng còn sống |
...
| **Đổi bộ não mà quên đổi phạm vi dữ liệu** | Não phụ nhận y nguyên prompt của Claude: đo thật một lượt đi ra 37.282 byte, trong đó có hồ sơ đời tư (giờ dậy, nghề, nơi ở) và số dư từng ví — sang máy một nhà miễn phí mà admin chưa từng đọc điều khoản | Prompt gửi ra ngoài phải CẮT ĐƯỢC theo khối, nên `brief` tách khỏi `them` (dò chuỗi con thì hỏng lặng lẽ). Mức `fallback.riengTu`, mặc định `canTrong`. Cắt khối nào thì phải NÓI với model là đã cắt (`_bao_da_cat`) — không nói thì nó đọc SYSTEM.md thấy dặn "dùng Bức tranh hiện tại", tìm không ra, rồi bịa số. Soi bằng `fallback.py xem-goi`, ca thử canh bằng cách bắt gói tin |
...
| **Hai distro chung một uid → chung một cgroup** | 25/09: terminal mở cả ngày mà CEO câm. Máy ảo CÓ sống, task giữ CÓ chạy — nhưng `user@1000.service` của Ubuntu chết ngay lúc bật với `219/CGROUP`, nên không poller, không cron, không hẹn giờ. Nguyên nhân: `hop` trong hộp `openclaw` cũng uid 1000, hai distro chung một cây cgroup của máy ảo, hộp dựng trước ba giây và chiếm chỗ. Hôm trước Ubuntu tới trước nên chạy — cuộc đua, không phải cấu hình. `travis health` BIẾT (im lặng 23 giờ) nhưng không nói được với ai, vì bộ báo nằm trong chính thứ đã chết. Và lúc sống lại, poller chỉ báo "có 2 tin cũ" — admin phải tự nhớ mình đã hỏi gì | Mỗi distro một uid (`hop` → 2000, xem `hop/README.md`); `ops/soatHop.py` soát va uid. Cách bắt: `journalctl -b 0 -u user@1000` trong Ubuntu — có `219/CGROUP` là dính. Poller nay DẪN LẠI giờ + nội dung tin bị bỏ. Cùng họ với dòng trên: **thứ canh không được nằm trong thứ nó canh** — nên có `ops/canhHe.sh`, unit của ROOT (`companyspec-canh.timer`, 2 phút): user manager hay poller chết thì dựng lại, ghi sổ, poller lên thì báo admin. Đã thử phá cả hai nhánh (dừng poller, dừng `user@1000`) — hệ tự lên lại trong một lượt canh |
| **Luật câu chữ chỉ sống trong prompt** | SYSTEM.md cấm Markdown từ ngày đầu, 18/08 thêm cả ví dụ ĐỪNG/HÃY. Đo 26/09 trên hội thoại thật: 7/41 tin vẫn có khối ```, backtick quanh tên file, đường kẻ `---` — Telegram hiện nguyên dấu giữa câu. Bộ evals có soát, nhưng chỉ soát lúc có người chạy nó; admin thì đọc mỗi ngày | Cùng họ `_so_khong_nguon`: thứ tra được bằng máy thì chặn bằng máy, ở CỬA RA (`session.bo_markdown` trong `build_reply`). Chỉ gỡ dấu, không đụng chữ — cố ý bỏ qua `*`/`_` đơn vì đoán sai là hỏng phép nhân và snake_case. Lưu vào lịch sử bản ĐÃ gỡ, để phiên sau không tự học lại. Mẫu neo theo dòng dùng `[ \t]`, đừng dùng `\s` — `^\s*` nuốt dấu xuống dòng và dán hai dòng vào nhau (ca `testCeoReplyHasNoMarkdown` bắt được ngay lần đầu) |
...
| **Cùng một bài học, vá một file quên file kia** | `proc.stderr.strip()[:400]` trong `gateway/cli/dispatch.py` — cắt log từ ĐẦU. Bài học "cắt từ đuôi" đã có sẵn trong bảng này và đã vá ở `gateway/telegram/poller.py` từ lâu, nhưng chưa ai vá ở dispatch. Mỗi lần một company chết là mất đúng dòng cuối nói hỏng vì cái gì | Đọc bảng này thôi CHƯA ĐỦ — phải `grep` xem còn chỗ nào cùng họ. Một bài học chỉ vá ở nơi nó xảy ra thì nó sẽ quay lại ở nơi khác |
| **Bộ quét phụ thuộc mù với import dạng gói** | Thêm luật C4.2 (`core/` không được phụ thuộc ra ngoài) xong, thử phá bằng `from ops import session` — codemap KHÔNG BẮT. Bản cũ chỉ so tên module PHẲNG, nên `from ops import session` và `from core.policy import decide` đều vô hình. Cả repo trước đây dùng import phẳng nên chưa ai gặp | Hàng rào mới phải được **THỬ PHÁ**, không phải đọc mã rồi tin. Nếu tớ chỉ đọc `codemap.py` thì đã kết luận là nó canh được. Nay `phu_thuoc()` soát cả tên gói gốc, mọi đoạn có chấm, và tên lôi ra trong `from X import Y` |
...
| **Bộ đo tự KIẾM QUYỀN cho hệ** | `trackRecordRows` đếm cả `reg_ evl_ e2e_ demo_`. Đo 20/09: trong 227 dòng của `travisSelfTestCompany.recordWrite` có 194 dòng `e2e_` và 33 dòng `demo_` — KHÔNG một lời gọi thật nào, mà bảng mức tự chủ in ra mức 2. `nhacCompany.dsNhac` cũng có 136 dòng `reg_` góp vào mức 4. Lọc xong thì `xuongCompany.nhanViec` tụt 77%→20% và `ghiBuoc` 69%→0%: tỉ lệ đạt đẹp phần lớn là của bộ đo | Tiền lệ `evl_`/`reg_` đã có hai lần trong bảng này, nhưng cả hai lần con số chỉ đi vào một BÁO CÁO. Lần này nó đi vào một QUYẾT ĐỊNH QUYỀN HẠN — cánh cửa mở được bằng `python3 tests/run.py` chạy vài lần. Lộ ra đúng lúc `core/policy` bắt đầu đọc con số ấy, tức là lúc ba ca e2e canh cửa duyệt đồng loạt đỏ. Bài học: **một phép đo bẩn nằm im cho tới ngày có ai đó dùng nó để quyết định.** Trước khi nối một con số vào cửa quyền, hỏi "ai đã ghi những dòng này" |
| **Cùng một luật, BA thân, lệch nhau một ngày** | "Điều này còn đúng không" được hỏi ở ba nơi với ba phép so khác nhau: `core/memory.recall` (SQL, so `expiresAt > utcNow()`), `MemoryItem.isExpired` (Python, cùng kiểu), và `session.profile_block` (so ngày VN). Hai cái đầu so chuỗi 10 ký tự `"2026-10-15"` với mốc 20 ký tự `"2026-09-20T05:00:00Z"` — chuỗi ngắn là tiền tố nên LUÔN nhỏ hơn, tức là `[đến 15/10]` rụng ngay 00:00 ngày 15/10 thay vì cuối ngày. Cộng thêm lệch múi giờ (UTC vs +07:00). Đo 20/09: **3 trên 4 mốc trong ngày cho kết quả khác nhau**, và cái sai nghiêng về hướng MẤT trí nhớ của admin | Định nghĩa gốc xuống tầng thấp nhất (`contracts.isStillValid`), ba nơi cùng `import` về; bản SQL bị ca **ma trận** ép nói giống bản Python. Chính ma trận ấy bắt thêm một lỗ không ai thấy: `expiresAt = ""` **ghi vào được mà không bao giờ đọc ra** — SQL coi rỗng là đã rụng, Python coi là không hạn. Nay `validate()` chặn ở cửa vào. Cùng họ với `_VERIFICATION_ACCEPTABLE` và `canWhitelist` viết ở ba nơi. Và bài học riêng: **ngày thì so với NGÀY**, còn múi giờ là sự thật của thế giới ngoài nên NGƯỜI GỌI đưa vào, core không tự đoán |
| **Đặt chốt canh lên chính cánh cửa nó canh** | Khai `autonomyOptIn` cho `travisSelfTestCompany.recordWrite` — năng lực mà ba ca `TestWritePathNeedsASignature` dùng để chứng minh việc GHI thì dừng lại và hỏi. Mở đường tự chạy cho nó là làm ba ca ấy xanh theo cửa đang mở | Năng lực nào đang làm CHỐT CANH cho một luật thì không được là nơi thử nới chính luật đó. Tách ra: `recordWrite` giữ vai chốt, `recordWriteAuto` mang `autonomyOptIn`. Cách bắt: sửa xong thấy ca thử đỏ thì hỏi "ca này canh cái gì", đừng hỏi "sửa ca thế nào cho xanh" |
| **Ghi đè `identity` làm TẮT ĐIỆN một luật, mà đèn báo vẫn sáng** | Cho cron một danh tính xong, `dispatch` gán `identity = IssuedBy.employee` mỗi lần cổng employee cho qua. Vì `employeeId` nay LUÔN có, phép gán ấy chạy cho MỌI lời gọi — và `_ruleScheduledTriggerIsReadOnly` mở đầu bằng `if request.identity is not IssuedBy.scheduledTrigger: return None`. **S3 không bao giờ chạy nữa.** Đo thật: một lời gọi `--issued-by scheduledTrigger` GHI vào sổ chi tiêu đi lọt với `allowWithVerify · whitelist` — thoát nhờ shell thiếu `NOTION_TOKEN` | Ba ca thử canh S3 **vẫn xanh suốt**, vì chữ "S3" lúc ấy do dispatcher tự ghép vào câu trả lời chứ không còn do Policy sinh ra. Đó là dấu hiệu cần nhớ: **ca thử đòi một CHUỖI thì bất kỳ ai cũng ghép được chuỗi đó**. `identity` nói AI PHÁT RA việc; ai ĐỨNG TÊN là câu khác, đã có `employeeId`. Đừng ghi đè. Ca mới gọi dispatcher THẬT bằng một employee CÓ quyền ghi và đòi Policy nói không — cộng đòi lý do có trong SỔ, không chỉ trong câu trả lời |
| **Ca thử dò CHUỖI vẫn xanh khi logic bị lật ngược** | `testSignedScheduleIsNotBlockedByTheEmployeeGate` đòi hai chữ `hen_ok` và `scheduler` cùng có mặt trong 200 ký tự quanh một mỏ neo văn bản. Lật ngược hoàn toàn thành `"scheduler" if hen_ok else "ceo"` thì **hai chữ ấy vẫn còn nguyên** — ca xanh, cơ chế hẹn giờ chết. Nó canh được mọi thứ trừ điều nó nói nó canh | Dò chuỗi trong mã nguồn là canh TRIỆU CHỨNG. Thay bằng hành vi: gọi cổng thật, đòi kết quả khác nhau giữa hai đầu vào. Cùng họ với `_nguonDispatch` trong bộ diễn tập — nếu phải canh "tham số X có tới cổng không" thì gọi hai lần với hai giá trị X và đòi hai kết quả KHÁC nhau; X không tới nơi thì hai kết quả giống nhau và ca đỏ |
| **Dọn cái THẺ mà để lại cái QUYỀN** | Vá "demo tự cấp quyền" bằng cách bắt từng bộ đo tự dọn `approvalRequest`. Nhưng phòng tuyến ấy nằm ở BA nơi, mỗi nơi phải tự nhớ gọi — và một nơi đã quên trên nhánh thoát sớm. Cái quyền đứng do sự cố đẻ ra thì vẫn sống, và còn bị commit vào repo | Cửa thật chỉ có MỘT: `approvals.whitelist_add`. Chặn ở đó — phiếu mang nhãn bộ đo KHÔNG BAO GIỜ đẻ ra quyền thường trực — thì phủ luôn lượt bị Ctrl-C, bộ đo thứ tư chưa ai viết, và mọi lần quên dọn về sau. Hỏi "cửa thật ở đâu" trước khi đi vá từng lối vào. Và khi vá một sự cố, hỏi tiếp: **thứ nó ĐẺ RA đã được thu hồi chưa**, hay chỉ dấu vết bị xoá |
| **Bản trình diễn tự cấp cho mình một quyền thường trực** | `travisDemo.py` cố ý đi qua cổng duyệt THẬT — đó là điểm của nó. Nhưng nó không dọn, nên phiếu duyệt nằm lại: đo 21/09 có **15 thẻ `demo_`** trong sổ. Phiếu duyệt KHÔNG nằm yên — nó hiện thành một cái thẻ CÓ NÚT trên Telegram, và admin đã bấm "luôn cho phép" một cái → đẻ ra quyền đứng THẬT `wl_7f5e254bdf53c200` cho `travisSelfTestCompany.recordWrite` | Lần này vô hại vì company ấy `internal: true` nên CEO không gọi được (C5) — nhưng vô hại do MAY, không do thiết kế. `tests/regression` và `tests/drills` đều tự dọn; chỉ demo quên. Nay `travisDemo.don_dep()` xoá phiếu của **lượt chạy mình** (không xoá mọi dòng `demo_` — đó là dấu vết của lượt khác). Ca `testApprovalLedgerHasNoLingeringTestCards` canh chung cho cả ba. Cùng họ "ca thử tự chạy thật việc GHI" |
| **Nguồn lưu lượng LỚN NHẤT lại chạy vô danh** | Nối employee xong, đo lại: 17/49 lời gọi thật mang `employeeId`. 31 trong 32 dòng thiếu là `trc_cron_`. Mặc định chỉ đặt `ceo` cho `issued_by == ceo`, nên **cron chạy vô danh** — mà cron là nguồn lớn nhất của hệ: **5.304 lời gọi trong ba tuần**. Sổ ghi đủ thứ về chúng, trừ câu §30 "ai làm" | Và nó là nguồn đáng có tên NHẤT: chạy lúc admin ngủ, 15 phút một lần, không ai nhìn. Thêm `employees/scheduler/` — CHỈ ĐỌC, biến S3 từ một hàng rào (luật) thành hai (luật + hồ sơ không có `modify` để dùng). Cách bắt: đếm `employeeId IS NULL` trên sổ thật, đừng tin là "đã nối xong" |
| **Siết chặt hơn luật, ở một tầng khác** | `scheduler` khai đúng `database.read` vì ba tuần qua cron chỉ chạm sổ. `testPolicyParity` đỏ ngay: **core nói CHO, dispatch nói KHÔNG** với 10 năng lực `read` khác (`searchCompany.traNhanh`, `browserCompany.kiemTraTrang`…). S3 viết *"việc định kỳ chỉ được phép ĐỌC"* — không viết *"chỉ được đọc sổ"* | Siết hơn luật nghe như cẩn thận hơn, thật ra là dựng một **luật THỨ HAI không ai biết**: lịch trỏ vào `kiemTraTrang` sẽ chết lặng lẽ, còn người đi tìm thì đọc S3 rồi kết luận nó phải chạy được. Hai tầng BẤT ĐỒNG nghĩa là một trong hai sai — đừng mặc định tầng chặt hơn là tầng đúng. Bất biến đúng ở đây: **đọc được MỌI thứ, làm thì không gì cả**; hàng rào thật nằm ở `cannot` |
| **Chặn sớm hơn làm MẤT lý do thật** | Cổng employee chạy trước Policy, nên khi cron có danh tính thì nó che mất S3: câu trả về chỉ nói *"`scheduler` không được phép database.modify"*. Đúng, nhưng không nói vì sao CRON nói riêng bị cấm — mà đó mới là điều người đọc log cần | Hai bộ ca thử bắt ngay, và cả hai đòi chữ **"S3"** trong câu trả lời chứ không đòi một mã trạng thái. Giữ cả hai lớp, NÓI CẢ HAI lý do. Và ca thử thì trói vào Ý NGHĨA (*"có nói không, và có giải thích được không"*), đừng trói vào mã trạng thái — trói vào mã thì mỗi lần thêm một lớp phòng thủ là một lần ca đỏ oan |
| **"Bỏ qua" bị đọc thành "đã khỏi"** | `decideNotification` hỏi *"lần này có hỏng không?"* rồi coi MỌI thứ không-hỏng là đã khỏi — kể cả `skipped`. `calendarWatch` chạy 15 phút/lần với `quietIfEmpty` nên lịch trống là `skipped`, mà người gọi lại cố ý bỏ qua `skipped` khi tìm trạng thái trước (để một lượt im không xoá dấu vết lần hỏng). Hệ quả: dòng `failed` lúc 08:52 nằm đó VĨNH VIỄN và cứ 15 phút admin nhận *"Đã chạy lại được (hỏng 1 lần liên tiếp trước đó)"*. Sổ đếm **67 tin** loại này từ 17/08 | Trớ trêu: đây đúng là "21 tin lúc nửa đêm" mà chính hàm ấy sinh ra để chặn — chỉ khác là tin BÁO TIN MỪNG, và nó dạy admin bỏ qua thông báo y hệt. "Đã khỏi" phải là KẾT LUẬN CÓ BẰNG CHỨNG: lượt này thật sự CHẠY ĐƯỢC. Dùng danh sách TRẮNG (`ok`, `completed`) chứ không danh sách đen — trạng thái mới quên khai thì rơi vào im lặng, sai về phía ít ồn hơn. Cùng họ `verification.skipped ≠ passed` |
| **Bộ đo phát minh nhãn mới mà quên ĐĂNG KÝ** | Bộ diễn tập tự đặt `TRACE_PREFIX = "drl_"` nhưng không thêm vào `TEST_TRACE_PREFIXES`. Bài `hong` cố ý làm `failOnPurpose` hỏng bốn kiểu → event bus (nối cùng ngày) thấy trong `taskLog`, tưởng sự cố THẬT, nhắn admin 5 dòng *"failOnPurpose hỏng — báo admin, chờ admin quyết"*. Bộ đo tự báo động về chính nó | Bẫy này đã ghi HAI lần trong bảng và vẫn dính, vì **đọc bảng không chặn được việc phát minh một nhãn mới**. Nay nhãn LẤY TỪ danh sách chung kèm `assert` ngay lúc nạp: chưa đăng ký thì file gãy ở bàn làm việc, thay vì im lặng lúc 22 giờ trên máy admin. Bài học: mỗi lần thêm một NGUỒN LƯU LƯỢNG mới, hỏi "ai đang đếm dòng này, và họ có biết nó là của bộ đo không" |
| **Vá trong repo, nhưng thứ ĐANG CHẠY nằm ở máy khác** | Vá `hop/tho.py`, ca thử xanh, commit sạch — mà thợ vẫn spam duyệt. Vì thợ chạy trong distro WSL **`openclaw`**, ở `/home/hop/tho.py`, một BẢN SAO từ 18/09. Không có đường đồng bộ nào; nó lặng lẽ cũ đi. Admin phải hỏi lần thứ hai tớ mới biết | Sửa xong phải hỏi: **"thứ đang chạy có phải thứ tớ vừa sửa không?"** Cách bắt trong 5 giây: `wsl -d openclaw -- grep -c '<tên hàm mới>' ~/tho.py` — ra 0 là chưa tới. Chép đè thì SAO LƯU trước và `diff` với bản repo trước khi vá, để biết hộp có sửa riêng gì không (lần này chỉ khác một dòng chú thích). Cùng họ với "hai bản của cùng một luật", nhưng ở đây hai bản nằm trên hai HỆ TỆP |
| **`datetime('now')` của sqlite không khớp định dạng đang lưu** | Đếm bằng `startedAt >= datetime('now','-24 hours')`. Sqlite trả `YYYY-MM-DD HH:MM:SS` (dấu CÁCH), còn sổ lưu `YYYY-MM-DDTHH:MM:SSZ` (chữ T). `' '` < `'T'` nên phép so nuốt luôn cả những ngày trước — cửa sổ rộng hơn 24 giờ thật. Tớ đã báo admin **330/36/294**, con số đúng là **234/15/219** | Cái sai này nghiêng về phía PHÓNG ĐẠI, nên nó không làm ai nghi ngờ — kết luận vẫn đúng, chỉ có con số to lên. Dựng mốc bằng Python với ĐÚNG định dạng đang lưu (`strftime('%Y-%m-%dT%H:%M:%SZ')`), đừng để sqlite tự sinh. Cùng họ "so chuỗi ngày nguyên bản". Mã thật không dính (đã grep); chỉ `poller.py:110` ghi `sentAt` bằng `datetime('now')` — vô hại vì **không ai đọc cột đó**, nhưng nó là một định dạng lạc lõng đang nằm chờ |
| **Chiều `environment` khai khắp nơi mà chưa từng tới cổng** | `dispatch` gọi `employee.mayDo(resource, action)` **không truyền `environment`**, nên phạm vi khai trong MỌI hồ sơ employee là chữ chết. Hỏng CẢ HAI CHIỀU cùng lúc: `forge.mayDo(repository, modify)` → **False**, thợ code bị cấm sửa code (quyền CÓ phạm vi thành vô dụng); `ceo.mayDo(...)` → **True kể cả `production`** (quyền KHÔNG phạm vi thành vô hạn). Người điều phối có quyền ghi RỘNG HƠN thợ code | Không ca thử nào trong 290 ca bắt được, vì không ca nào gọi `mayDo` đúng cách dispatcher gọi. Bài diễn tập `suacode` bắt ngay lần chạy đầu. Sửa: dispatch truyền `environment` (mặc định `dev`, có cờ `--environment` để chạm production là việc phải KHAI RA), và `ceo` được siết về `dev`. Dấu hiệu nhận ra loại này: **quyền của người điều phối rộng hơn quyền của thợ** — luôn là quyền khai lỏng |
| **Tầng đã dựng mà không ai đi qua** | `employees/` có 5 hồ sơ, `mayDo()` thi hành đúng, ca thử xanh — nhưng đo 20/09: **6 trên 6.146 lời gọi thật trong 7 ngày** mang `employeeId`, tức **0,1%**. CEO gọi thẳng dispatcher với quyền NGẦM ĐỊNH vô hạn, chưa từng bị `mayDo()` soát lần nào. Hệ có hai hình — hình trên giấy (Task→Employee→Policy) và hình đang chạy (CEO→Policy) — và chỉ hình thứ hai mới đúng | Ca thử xanh chỉ chứng minh tầng ấy CHẠY ĐƯỢC, không chứng minh có ai DÙNG. Cách bắt rẻ: đếm xem trên sổ thật có bao nhiêu dòng đi qua nó. Sửa: khai `employees/ceo/employee.yaml` với quyền **đo từ cả danh mục**, không từ lưu lượng (lưu lượng nói thứ hay dùng, manifest nói thứ có thể dùng — mà "ít dùng" ở đây thường là "nặng"); mặc định `employeeId` đặt ở DISPATCHER chứ không ở prompt, vì CEO tự gõ lệnh và `guard.py` chỉ cho vài cờ. Đối chiếu trước bằng `testCeoAsEmployee` |
| **`riskTier` bị dùng thay cho `action`** | `Capability.touches` suy hành động hoàn toàn từ rủi ro: `irreversible` → `delete`. Nên `panharmonCompany.dangBai` — ĐĂNG một bài lên web — bị soát quyền như thể nó XOÁ CẢ KHO, và muốn CEO đăng được bài thì phải cấp `repository.delete`, đúng quyền mà `forge`/`sentinel` bị cấm vì lý do hoàn toàn khác. Bằng chứng nó là lỗ thật chứ không phải chuyện chữ: **bốn trong bảy** giá trị `ActionKind` (`create`, `publish`, `send`, `deploy`) chưa từng xuất hiện được — kể cả `publish`, thứ bảng duyệt §7 gọi tên thành một dòng riêng | "Nặng cỡ nào" và "làm gì" là HAI câu. Suy câu này từ câu kia thì có ngày ra một câu vô nghĩa. Cho manifest khai `action:` như đã khai `resource:`, không khai thì vẫn suy như cũ (87 năng lực còn lại không đổi hành vi). Cách bắt: liệt kê giá trị enum nào KHÔNG BAO GIỜ dựng ra được — enum chết là dấu hiệu một khái niệm đang bị nuốt vào khái niệm khác |
| **Đổi tên file, quên nơi GỌI tên cũ** | §36 đổi `gateway.py` → `session.py` (bắt buộc, vì có gói `gateway/`), nhưng phép thay `gateway`→`session` quét hơi rộng và trúng **bốn** chỗ: `poller.GATEWAY` trỏ vào `gateway/telegram/gateway.py` — file CHƯA TỪNG tồn tại, nên **mọi tin nhắn admin ra "Hệ gặp lỗi khi xử lý"**; `session.py` tự gọi lại chính nó để làm mới Bức tranh, `Popen` nuốt cả stdout lẫn stderr nên hỏng TUYỆT ĐỐI im lặng; `codemap` tìm ca thử ở `ops/evals/` cũ, có `if isfile()` bao ngoài nên **hàng rào tự tắt mà vẫn in "SOÁT LUẬT: sạch"**; và `gateway.local.yaml` bị đổi tên rời khỏi file nó đè | Nặng nhất nằm im **không ai biết** vì admin chưa nhắn lần nào sau khi đổi tên — journal có ĐÚNG 0 dòng `gateway lỗi`. Ba bộ kiểm đều xanh suốt: không ca nào CHẠY THỬ đường poller đẻ ra. Vá bằng `tests/regression/testPathsPointAtRealFiles.py` — quét AST mọi `os.path.join(ROOT, …)` viết cứng, cộng một ca **chạy thật** cửa vào và đòi nó trả JSON. Cửa thoát phải KHAI TÊN kèm lý do, không dùng mẫu rộng. Đã thử phá: đổi lại tên sai thì ca đỏ ngay |
| **Ghi vào được mà không bao giờ đọc ra** | `expiresAt = ""` cất được bình thường: `remember()` trả về, không lỗi gì. Rồi `recall()` không bao giờ thấy nó nữa — SQL coi chuỗi rỗng là ĐÃ RỤNG, Python coi là KHÔNG HẠN. Một điều admin dặn ta nhớ, biến mất ngay lúc cất | Không ai đọc ra bằng mắt; bắt được nhờ một ca **ma trận** so SQL với Python trên chín cặp (giá trị, mốc). Viết hai thân của một luật thì phải viết luôn cái ma trận ép chúng nói giống nhau. Và chặn hình dạng hỏng ở CỬA VÀO (`validate()`), đừng để bộ đọc phải đoán |
| **Phiếu chỉ ghi sổ, không gánh việc** | Cách nối Secret broker dễ nhất: cấp phiếu để có dòng trong sổ tra, rồi vẫn bơm secret theo danh sách manifest như cũ. `auditTrail()` trả lời rất đẹp cho câu §30 "khoá nào đã bị chạm", trong khi cái phiếu không kiểm soát gì | Dựng `allowedSecretNames` **TỪ PHIẾU**, để phiếu bị từ chối là khoá vắng mặt thật. Cách tự hỏi: "gỡ cái bản ghi này đi thì hành vi có đổi không?" — không đổi thì nó là trang trí. Cùng họ với `pattern:`/`anyOf` khai mà không ai soát, và với `whitelistScope: []` |
...
python3 tests/drills/run.py --liet-ke           # năm bài DIỄN TẬP SỐNG, xem tốn gì
python3 tests/drills/run.py                     # chạy chúng — 0đ, dây chuyền THẬT
python3 tests/run.py --fast                     # bỏ ca gọi tiến trình con (~1s)
python3 tests/run.py --suite integration        # chỉ ca đi hết dây chuyền
python3 gateway/cli/travis.py health                    # hệ còn sống không, im lặng bao lâu
python3 gateway/cli/travis.py why <taskId>              # VÌ SAO lời gọi đó được phép
python3 gateway/cli/travis.py autonomy                  # mức tự chủ ĐÃ KIẾM được, và ai ĐANG tự chạy
python3 gateway/cli/travis.py secrets                   # khoá nào đã bị chạm, ai chạm, để làm gì (§30)
python3 gateway/cli/travis.py employees                 # ai làm được gì, ai bị cấm gì
python3 gateway/cli/travis.py brains sensitive          # mức dữ liệu này gửi được cho ai
python3 gateway/cli/travis.py verify                    # tự kiểm repo bằng Verification Engine
python3 gateway/cli/travis.py acquire lab/quarantine/x  # soi repo lạ, KHÔNG chạy nó
python3 ops/namingAudit.py                      # từ điển tên; --check nếu chỉ muốn biết sạch hay không
python ops/soatHop.py                           # ⚠ CHẠY TỪ WINDOWS — hộp openclaw có đang chạy đúng mã repo không
python3 ops/codemap.py --check                  # luật kiến trúc
python3 gateway/cli/dispatch.py list                    # danh mục company thật
...
python3 gateway/telegram/session.py soat-so-tay --thieu            # lượt nào router KHÔNG nạp sổ tay nào
python3 brains/fallback.py trang-thai                         # đang chạy bộ não nào, chuỗi dự phòng ra sao
python3 brains/fallback.py kiem                               # hỏi THẬT từng nhà: tên model còn sống không
python3 brains/fallback.py xem-goi "câu thử"                  # prompt SẼ gửi ra nhà ngoài — in ra, không gửi
python3 tests/evals/nao_thu.py                          # ca khung xương não phụ + hội đồng — 0đ, không cần mạng
python3 gateway/cli/scheduler.py hen                          # soát hẹn giờ: nhắc tới giờ + phiếu hẹn admin đã ký
```
...
```bash
python3 tests/evals/run.py --liet-ke      # xem có ca nào, không tốn gì
python3 tests/evals/run.py --only chi-ck  # một ca ≈ $0,02–0,10
python3 tests/evals/run.py                # cả bộ — tốn tiền thật, xem trước bằng --liet-ke
python3 tests/evals/run.py --nao phu      # đo BỘ NÃO DỰ PHÒNG thay vì Claude
```
...
cho mọi ca. Prompt dựng y hệt bản thật, gồm cả sổ tay mà router chọn. Company không hề chạy: dispatcher giả nằm ở `tests/evals/shim/`, bộ chạy
...
- **CEO không có tool `Read`, `WebFetch`, `WebSearch`, `Write`.** Mở ra là nó
  đọc được `ops/.env`. Ảnh do `gateway/telegram/media.py` đọc hộ bằng tiến trình riêng;
...
- **`dispatch.py` là cổng duy nhất** ra mọi company (T2). Không có đường vòng —
  kể cả bộ não dự phòng (`brains/fallback.py`), vốn chỉ có đúng một công cụ `goiCompany`
