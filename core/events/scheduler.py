...
cách thu hẹp: chỉ đọc, chỉ lịch đã khai trước, không LLM trừ khi thật cần.
    ...
    python3 core/events/scheduler.py run          # chạy lịch tới hạn (timer gọi)
    python3 core/events/scheduler.py run --force morningReport   # chạy tay để thử
    python3 core/events/scheduler.py list         # xem lịch và lần chạy gần nhất
"""
import argparse
import calendar
import json
...
# §36 — sau khi ops/ tan ra, approvals không còn là hàng xóm.
sys.path.insert(0, os.path.join(
    os.path.dirname(os.path.dirname(HERE)), "core", "policy"))
...
# ═══════════════════════════════════════════════════════════════════════
# GỬI TIN VÀ BÁO CÁO CHI PHÍ ĐƯỢC TIÊM VÀO — KHÔNG import ở đây
# ═══════════════════════════════════════════════════════════════════════
#
# C4.2 — `core/` là LUẬT, không phải người vận chuyển. File này quyết định
# LÚC NÀO chạy gì và có nên nhắn không; việc nhắn BẰNG CÁCH NÀO là của
# `gateway/telegram`.
#
# Trước 2026-09-20 nó `import telegram` thẳng, và khi §36 đẩy file này vào
# core/ thì codemap bắt ngay — đúng việc của codemap. Cách chữa KHÔNG phải nới
# luật, mà là tách đúng chỗ: động cơ ở đây, dây dẫn ở session.
#
# Người chạy thật là `gateway/cli/scheduler.py`; nó nối hai đầu rồi gọi vào.
_notify = None
_costSnapshot = None
_spendSnapshot = None


def configure(*, notify, costSnapshot, spendSnapshot) -> None:
    """Nối động cơ với thế giới bên ngoài. Gọi TRƯỚC khi chạy."""
    global _notify, _costSnapshot, _spendSnapshot
    _notify, _costSnapshot, _spendSnapshot = notify, costSnapshot, spendSnapshot


def _send(chatId: str, text: str) -> dict:
    """Gửi một tin. Chưa nối thì NÓI RÕ, đừng nuốt.

    Nuốt ở đây là kiểu hỏng tệ nhất: lịch vẫn chạy, sổ vẫn ghi "đã gửi", và
    admin không nhận được gì — không có dòng lỗi nào để lần ra (O10).
    """
    if _notify is None:
        raise RuntimeError(
            "scheduler chưa được nối với đường gửi tin. Chạy qua "
            "`gateway/cli/scheduler.py`, hoặc gọi `scheduler.configure(...)` "
            "trước.")
    return _notify(chatId, text)
...
def don_dong_cron() -> int:
    ...
    · chỉ `traceId LIKE 'trc_cron_%'` — trí nhớ của CEO đọc taskLog theo trace
      của PHIÊN nó (`session.viec_da_lam`), tiền tố khác hẳn. Đo 01/09: số dòng
