...
#: Mức riêng tư của `nao.riengTu` → mức phân loại dữ liệu.
#:
...
#: nghĩa của nó. Bắt được ngay khi chạy `nao.py trang-thai` và đọc dòng mô tả
...
def filterProviderChain(chain: list, classification: DataClassification,
    ...
    được phép dùng nó là `ops/evals/`.
