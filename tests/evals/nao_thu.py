...
"""Thử bộ não dự phòng bằng một nhà cung cấp GIẢ. Không tốn đồng nào, không cần mạng.
    ...
    python3 tests/evals/nao_thu.py

VÌ SAO PHẢI CÓ: `tests/evals/run.py` đo CEO thật — nó gọi model thật, tốn tiền
...
# §36 — ops/ đã tan ra: não phụ sang brains/, phiên CEO sang gateway/telegram/.
sys.path.insert(0, os.path.join(ROOT, "brains"))
sys.path.insert(0, os.path.join(ROOT, "gateway", "telegram"))
import fallback  # noqa: E402

# ĐÁNH NHÃN MỌI LỜI GỌI CỦA BỘ ĐO.
#
# `fallback.chay` nhận `trace_id`; ca thử không truyền nên dispatch tự sinh `trc_…`,
...
def _c1(port):
    ...
    kq = fallback.chay("chào em", system_prompt="bạn là CEO",
...
def _c2(port):
    ...
    kq = fallback.chay("ghi giúp em", system_prompt="bạn là CEO",
                  cfg=cau_hinh(port, [{"nha": "gia", "model": "m1"}]))
...
def _c4(port):
    ...
    kq = fallback.chay("hỏi tí", system_prompt="bạn là CEO",
...
def _c5(port):
    ...
    assert "brains/fallback.py kiem" in kq["result"], kq["result"]


@ca("hết trần lượt thì nói thật là CHƯA XONG")
def _c6(port):
    KICH_BAN[:] = [{"goi": GOI_HONG} for _ in range(10)]
    kq = fallback.chay("làm mãi đi", system_prompt="bạn là CEO",
...
def _c7(port):
    ...
    kq = fallback.chay("hỏi tí", system_prompt="bạn là CEO", cfg=cfg)
    assert kq["is_error"], kq
...
def _chay_ceo_qua_nha_gia(port, muc, brief, them):
    ...
    session.llmClient_nap = lambda: cfg
    fallback.llmClient.nap = lambda path="": cfg
    fallback.che_do = lambda c=None: ("phu", "ca thử")     # không đụng sổ thật
    KICH_BAN[:] = [{"chu": "ok"}]
    session.run_ceo("50k ck ăn sáng", "phien-thu-rieng-tu", resume=False,
...
def _c18(port):
    ...
    for cau in ("họp chiến lược kênh youtube", "hop chien luoc kenh yt",
        ...
        assert session.goi_hop(cau), f"trượt: {cau}"
        assert "hoi-dong" in session.chon_so_tay(cau), cau
    for cau in ("cái này có phù hợp không em", "trường hợp anh nghỉ thì sao",
                "hợp đồng thuê nhà hết hạn khi nào", "50k ck ăn sáng"):
        assert not session.goi_hop(cau), f"dính oan: {cau}"
        assert "hoi-dong" not in session.chon_so_tay(cau), cau
...
def _c19(port):
    ...
    goc = fallback._con_tra_tien_duoc
    fallback._con_tra_tien_duoc = lambda: (False, "đã tiêu 100.000đ, chạm trần.")
    try:
        kq = fallback.chay("hỏi tí", system_prompt="bạn là CEO", cfg=cfg)
    finally:
        fallback._con_tra_tien_duoc = goc
    assert kq["is_error"], kq
    assert "tính tiền thật" in kq["result"], kq["result"]
