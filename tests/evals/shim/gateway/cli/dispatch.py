...
(`python3 gateway/cli/dispatch.py call …`), không soát các tham số phía sau. Một CEO
...
if not ROOT:
    ...
    print("shim: thiếu COMPANYSPEC_EVAL_ROOT — bản giả này chỉ chạy từ "
          "tests/evals/run.py.", file=sys.stderr)
    sys.exit(2)

sys.path.insert(0, os.path.join(ROOT, "gateway", "cli"))
