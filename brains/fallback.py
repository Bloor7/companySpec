...
BA CHẾ ĐỘ (registry/models.yaml → fallback.macDinh, admin đổi được lúc chạy):
...
nguyên: vẫn đúng một cổng `gateway/cli/dispatch.py`, vẫn đủ kiểm tra schema, riskTier,
phê duyệt, loop guard.

ĐÃ ĐO 2026-08-31 (`python3 tests/evals/run.py --nao phu`), và kết quả đúng như dự
...
DISPATCH = os.path.join(ROOT, "gateway", "cli", "dispatch.py")
...
def _cat(s: str, n: int) -> str:
    """Cắt và NÓI RÕ là đã cắt (cùng luật với session.cat_gon).
...
def chay(message: str, *, system_prompt: str, lich_su: list = None,
    ...
    #
    # `fallback.riengTu` quyết định prompt bị cắt tới đâu, và mức cắt quyết định
    ...
    if not chuoi:
        return {"result":
                ...
                "registry/brainPolicy.yaml, hoặc đặt `fallback.riengTu: canTrong` "
        ...
        try:
            ...
            return {"result":
                    ...
                    + " Đại ca kiểm khoá trong ops/.env hoặc chạy "
                      "python3 brains/fallback.py kiem.",
...
def _nap_env() -> None:
    ...
    """
    # §36 — `session` nay ở gateway/telegram/, không còn là hàng xóm của
    # file này (file này đã sang brains/).
...
def cmd_trang_thai(args) -> int:
    ...
    print("  Xem tận mắt: python3 brains/fallback.py xem-goi \"câu muốn thử\"")
...
def cmd_xem_goi(args) -> int:
    ...
    muc = session._muc_rieng_tu()
    conn = session.ceo_store()
    brief = session.brief_block(conn)
    sach, _ = session.so_tay_block(args.cau, False)
    conn.close()
    loi = open(session.SYSTEM_PROMPT, encoding="utf-8").read()
    danh_muc = session.danh_muc_block()
    ho_so = session.profile_block()
    ...
    print(f"Chế độ riêng tư: {muc} — {session.RIENG_TU[muc]}\n")
