...
1. CẦU NÀY HẸP HƠN `dispatch.py`, KHÔNG RỘNG HƠN.
   ...
   dạng với `ops/nao.py`, vốn chỉ có đúng một công cụ và dựng argv bằng tay.
...
Chạy:  python3 ops/cau.py            (nghe 127.0.0.1:8787)
Thử:   python3 ops/cau.py --tu-kiem  (không mở cổng, chỉ soát cấu hình)
"""
import argparse
import json
import os
...
def goi_dispatch(company: str, capability: str, inp: dict, tra: int) -> dict:
    """Dựng argv BẰNG TAY, không qua shell. Cùng lẽ với ops/nao.py."""
    proc = subprocess.run(
...
def moi_duyet(kq: dict, company: str, capability: str) -> None:
    ...
    Đi qua `ops/telegram.py`, đường ra duy nhất (T1). Gắn nhãn "HỘP" ở đầu vì
    ...
    try:
        ...
        nut = gateway.approval_keyboard(
            ma, gateway.can_whitelist(company, capability,
    ...
    def do_POST(self):
        ...
        if nang is None or capability not in nang:
            ...
            return self._tra(403, {
                ...
                "cachMo": "khai thêm vào CHO_PHEP trong ops/cau.py, có chủ ý"})
