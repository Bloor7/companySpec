...
- Kiểm tra bằng code trong `ops/dispatch.py` — model gọi qua nó, không thay nó được.
...
└───────────────────────────┬──────────────────────────────────┘
                            │ taskEnvelope
┌───────────────────────────▼──────────────────────────────────┐
│ dispatcher       ops/dispatch.py + hook chặn mọi đường khác.  │
...
- **T2.** dispatcher là điểm nghẽn cố ý (chokepoint): mọi lời gọi company đi qua
  ...
  thực — hook `PreToolUse` chặn mọi đường khác, và `ops/dispatch.py` gác cổng
  thật sự. Xem §12b.
...
- **C3.4 — Mở SQLite qua `lib/db.py`, không bao giờ gọi `sqlite3.connect` thẳng.**
  ...
  *Luật chung:* thứ gì cả hệ đều dùng và dễ làm sai thì phải có **đúng một chỗ
  làm đúng**, mọi nơi khác gọi vào đó (giống `ops/telegram.py` với đường ra).
...
- **L7.1 — Việc hỏng vẫn phải khai chi phí.** Một phiên LLM chết giữa chừng đã
  ...
  **Nhánh bị CẮT NGANG đã lấp (2026-08-14).** `--output-format json` im lặng
  ...
  `stream-json` + đọc từng dòng trong tiến trình cha (`lib/skillRun.py`): số
  ...
  **Con số nhánh này là ƯỚC LƯỢNG, và phải được gọi đúng tên.** Phiên chạy trọn
  ...
  bảng giá trong `skillRun.GIA`. Ba điều đã đo, đừng phát hiện lại:
...
| # | Câu hỏi | Chốt |
|---|---|---|
| Q1 | CEO chạy ở đâu, xác thực kiểu gì | Chạy thẳng trên máy local (WSL), CLI dùng gói Pro qua `claude.ai` auth có sẵn. `ops/poller.py` gọi bằng `subprocess`. |
...
> một chỗ, và chỗ đó không phải là prompt. Hiện thực là `ops/dispatch.py`, và
...
| 2026-08-03 | **Lát 4 — `scheduledTrigger`.** `registry/schedules.yaml` (3 lịch), `ops/scheduler.py`, systemd timer 15 phút. **S3 kiểm ở dispatcher chứ không tin file lịch.** Tách `ops/telegram.py` thành đường ra duy nhất (T1) | Cron chạy lúc admin ngủ, nên mọi guardrail dựa trên "hỏi admin" mất tác dụng. Bù bằng cách thu hẹp cửa: chỉ đọc, chỉ lịch khai trước, mặc định không LLM |
...
| 2026-08-02 | Bỏ n8n khỏi đường Telegram, thay bằng long polling (`ops/poller.py`) | Telegram webhook cần URL công khai HTTPS; máy nằm sau NAT. n8n giữ lại cho `scheduledTrigger` — loại chạy từ trong ra, không cần ai gọi vào |
...
| 2026-08-02 | Nới C2.3: schema được viết thẳng trong `companySpec.yaml` khi ngắn; cập nhật §8 cho khớp cấu trúc thật (`ceo/SYSTEM.md`, `ops/dispatch.py`, `ops/ask.py`) | Thực tế khác bản phác thảo — sửa hiến pháp trước (E1) |
